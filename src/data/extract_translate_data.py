...
def generate_fighter_stats(input_dir, output_file):
    ...
    cols = ['fight_id', 'f_1', 'f_2', 'winner', 'result', 'weight_class']
    ...
    agg = df_long.groupby('fighter_id').agg(
        full_name         = ('full_name', 'first'),
        age               = ('age', 'first'),

        weight_class      = ('weight_class', lambda x: x.mode()[0] if not x.mode().empty else np.nan),
        fighter_stance    = ('fighter_stance','first'),
    ...
    print(f"Resumo com estatísticas, categoria e estilo salvo em: {output_file}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Gera estatísticas por lutador UFC, categoria e estilo.")
